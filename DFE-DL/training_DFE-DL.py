...
with open('**/params_DFE-DL_training.json') as f: # args[1] should contain path to params.json file
    params = json.load(f)

test_seed = random.randint(1, 1000)       # set random seed for creating the testing set first
random.seed(test_seed)

full_dataframe = pd.read_pickle('**/RE dataset_one conformer.pkl')

# this can be obtained by MSS or random splitting
...
print('training set size',train_dataframe.shape[0],'validation set size',val_dataframe.shape[0],'test set size',test_dataframe.shape[0])

# CREATE DIRECTORY FOR SAVING/CHECKPOINTING
save = params['save']

PATH = '**/data/trained' # should contain path to subfolder where files will be saved
if PATH[-1] != '/':
    PATH = PATH + '/'
